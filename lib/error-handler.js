...
exports = module.exports = function(logger, config) {
  ...
  return function(err, req, res, next) {
    ...
    if (_.isObject(err) && _.isNumber(err.code) && err.code === 11000) {
      ...
      err.message = util.format('The %s provided already exists. Please try a different one.', field);
      err.param = field;
    }
    ...
    */
    ...
    if (config.showStack) error.stack = _.isUndefined(err.stack) ? new Error(err.message).stack : err.stack;
    ...
    // if (res.statusCode === 401) return res.redirect('/401');
    // if (res.statusCode === 403) return res.redirect('/403');
    // if (res.statusCode === 404) return res.redirect('/404');
    ...
    res.format({
      ...
      html: function() {
        ...
        if (req.flash) req.flash(level === 'warn' ? 'warning' : level, error.message);
        res.redirect('back');
      },
      json: function() {
