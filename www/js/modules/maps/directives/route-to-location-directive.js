angular.module('Maps').directive('routeToLocation', [
  '$rootScope',
  'MapsLoader',
  'RouteService',
  'MapsEvents',
  function($rootScope, MapsLoader, RouteService, MapsEvents) {

    function drawRoute(L, startLocation, destinyLocation, mapInstance, $scope) {
      if (!startLocation || !destinyLocation) return;
    ...
    function link($scope, element, attrs, ctrl) {
      MapsLoader.getMap.then(function(L) {
        var mapControllerScope = $scope.$parent.mapInstance ? $scope.$parent : $scope.$parent.$parent;
