...
Don't forget to bump the version numbers in config.xml - specifically `ios-CFBundleVersion` and `version`.

Then do`ionic build ios --release` which is what http://ionicframework.com/docs/guide/publishing.html says.
If you get a license accept issue then you can do `sudo xcodebuild -license accept`.

More generically, make sure you have "Generic iOS Device" selected in the top left, after the Square "Stop" button and then do "Product" "Archive"
