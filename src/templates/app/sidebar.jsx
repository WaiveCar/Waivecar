    ...
    if (auth.user.role === 'admin') {
      return (
        <div className="sidebar-admin">
          <h5 className="animated fadeIn">Admin <small>Menu</small></h5>
          <ul>
  ...
  getLink(link, i) {
    return (
      <li key={ i } className={ link.parent ? "has-parent" : "parent" }>
        <Link to={ link.path } className="nav-link animated fadeIn">
    ...
    return (
      <div id="sidebar" className={ this.state.open ? 'show' : '' }>
        ...
        <div className={ `sidebar-account${ this.state.account ? ' show' : '' }` }>
          <h5 className="animated fadeIn">Account <small>Menu</small></h5>
          <ul>
            { this.account() }
          </ul>
        </div>
        ...
        <div className="sidebar-nav">
          <h5 className="animated fadeIn">Application <small>Menu</small></h5>
          <ul>
            { this.nav() }
          </ul>
        </div>
