...
module.exports = class OrderService extends Service {
  ...
  static *createTimeOrder(booking, user) {
    ...
    try {
      ...
      yield notify.notifyAdmins(`Successfully charged ${ user.name() } for ${ minutesOver} minutes extra driving time | ${ apiConfig.uri }/bookings/${ booking.id }`, [ 'slack' ], { channel : '#rental-alerts' });
      log.info(`Charged user for time driven : $${ amount / 100 } : booking ${ booking.id }`);
    } catch (err) {
      log.warn(`Failed to charge user for time: ${ user.id }`, err);

      yield notify.notifyAdmins(`Failed to automatically charge ${ user.name() } for time driven: ${ err } | ${ apiConfig.uri }/bookings/${ booking.id }`, [ 'slack' ], { channel : '#rental-alerts' });
    }
